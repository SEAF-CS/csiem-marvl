...
subplot(1,4,j);
    ...
    t0=datenum(datestrings{1},'yyyymmdd');
    tt0=find(abs(timesteps-t0)==min(abs(timesteps-t0)));
    t1=datenum(datestrings{1+1},'yyyymmdd');
    tt1=find(abs(timesteps-t1)==min(abs(timesteps-t1)));

    tmp=ncread(ncfile,loadnames{j});
    tmp2=tmp(bottom_cellsHR,:);
    cdata1=mean(tmp2(:,tt0:tt1),2);clear tmp tmp2;
  %  d=tdat.D;
...
%     hold on;
   ...
   %  annotation('textbox',dim,'String',str,'FitBoxToText','on','LineStyle','none','FontSize',15);
    ...
    hold on;
     ...
     img_name ='./Phyto_biomass_map.jpg';
 saveas(gcf,img_name);