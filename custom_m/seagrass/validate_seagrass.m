...
ncfile.name='W:/csiem/Model/TFV/csiem_model_tfvaed_2.0/outputs/results/csiem_A001_20221101_20240401_WQ_WQ.nc';
...
time=timesteps;
t0=datenum(2022,12,1);
ts=find(abs(time-t0)==min(abs(time-t0)));
t1=datenum(2023,3,1);
tf=find(abs(time-t1)==min(abs(time-t1)));

tmp0=ncread(ncfile.name,'WQ_DIAG_MAC_MAC_AG');
model.MAC_A=tmp0(bottom_cellsHR,:);
...
for c=1:length(cell2p)
    modeldata(c,:)=model.MAC_A(cell2p(c),ts:tf);
    tmp=model.MAC_A(cell2p(c),ts:tf);
