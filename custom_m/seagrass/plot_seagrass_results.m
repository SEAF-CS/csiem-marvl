...
t1=datenum('20240101 00:00','yyyymmdd HH:MM');
...
vars={'WQ_DIAG_TOT_PAR','WQ_DIAG_MAC_MAC_AG','WQ_DIAG_MAC_MAC_BG','WQ_DIAG_MAC_MAC_FR'};

readdata=0;

if readdata
    for i=1:length(vars)
    tmp=ncread(ncfile(1).name,vars{i});
    output.(vars{i})=tmp(Bottcells,:);
    end
save('processed_seagrass.mat','output','-mat','-v7.3');
else
load('processed_seagrass.mat');
end


titles={'(a) photon captured','(b) above-ground biomass','(c) below-ground biomass','(d) fruit biomass'};
yls={'W/m^2','mmol C/m^2','mmol C/m^2','mmol C/m^2'};
ylims=[80 30 30 2];
time=timesteps;
...
for i=1:length(vars)
    ...
    for j=1:length(cellID)
        tmpdata=output.(vars{i})(cellID(j),:); %*factors(i);
        if i==1
