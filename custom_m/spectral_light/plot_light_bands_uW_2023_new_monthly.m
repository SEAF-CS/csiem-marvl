...
%scenario='SALCH4';
load(['W:\csiem\csiem-marvl-dev\others\CSIEM20_reporting\light\extracted_2023_light.mat']);
%load('extracted_for_Dan.mat');
field=load('E:\light_and_seagrass/field/light_summary.mat');
...
for b=1:length(bands_str)
    ...
    Ldir(b,:)=output.Kwinana.(varname).profile(dlayer,:);

    varname=upper(['WQ_DIAG_OAS_DIF_BAND',num2str(b)]);
    Ldif(b,:)=output.Kwinana.(varname).profile(dlayer,:);
end
...
datearray=datenum(2022,11:24,1);
...
f2=load('E:\light_and_seagrass\field\light_spetrum_KwinanaShelf_uW.mat');
...
%monthlymean(1,:)=monthlymean(1,:)*1.8;
   ...
   % set(hl,'Location','north','FontSize',6);
    %xlabel('Months');
	set(gca,'xlim',[0 15],'XTick',1:14,'XTickLabel',datestr(datearray,'mmm/yyyy'),'ylim',[0 200]);
    ylabel('\muW/cm^2/nm');
    title('monthly-average light intensity: modelled 2023')

axes('Position',pos1)
    datearray2=datenum(2022,11:24,1);
%monthlymean(1,:)=monthlymean(1,:)*1.8;
field.monthlymean(9,:)=NaN;
    hb2=bar(field.monthlymean(1:13,:),'stacked');
    for j=1:9
    ...
    %xlabel('Months');
	set(gca,'xlim',[0 15],'XTick',1:14,'XTickLabel',datestr(datearray2,'mmm/yyyy'),'ylim',[0 200]);
    ylabel('\muW/cm^2/nm');
...
for j=1:6 %length(d2)
   % t2p=find(abs(time-d2(j))==min(abs(time-d2(j))));
    plot(bands,monthlymean_ori(j,:)*100);
    hold on;
% 
...
pngname=[outdir,'model_KwinanaShelf_Fixed_uW_2023_new.png'];
