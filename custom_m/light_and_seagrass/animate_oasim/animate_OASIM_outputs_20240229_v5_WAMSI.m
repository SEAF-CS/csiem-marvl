...
if readdata
    ...
    for vv=1:length(allvars)
        ...
        if any([strfind(varname,'OAS'),   strfind(varname,'PHY_PAR'), ...
                strfind(varname,'TOT_EXTC')])                     %strfind(loadname,'OAS')
    ...
    save('./export_light_data_profiles_all_restart_08.mat','dataALL','-mat','-v7.3');
else

    load('./export_light_data_profiles_all_restart_08.mat');
end
...
%
outDir='.\Plots-20240229-v5-WAMSI\';
if ~exist(outDir,'dir')
    mkdir(outDir);
end

def.datearray=datenum(2022,1,3):1:datenum(2022,1,7);
...
t1=datenum('20220120 12:00','yyyymmdd HH:MM');
...
for i=tt+3:plot_interval:tt1
    ...
    xtickangle(angle);
    set(gca,'ylim',[0 2]);
    title('(a) Incoming irradiance at particular bands','FontSize',fs+4);
    ...
    plot(WL2,WamsiData(i,:),':','Color',[44,162,95]./255);
    hold on;
    ...
    plotdataR(5,:)=interp1(lambda_w,a_w,lambda_out);

    %    yyaxis left;
    plotdataR2=log10(plotdataR);
    ...
    set(gca,'xlim',lambda_range,'XTick',lambda_out,'XTickLabel',{'280','300','350','380','410','440','490','510','550','590','635','660','700','780','850','1100'});
    xtickangle(angle);
    set(gca,'ylim',[0 2]);hold on;
    text(850-50, 1.8,'a_w=3.74'); hold on;
    text(1100-50, 1.8,'a_w=43.25'); hold on;
    ...
    title('(c) IOP absorbance at particular bands @surface','FontSize',fs+4);
    ...
    ha1=area(tmp.date,plotdata2'); hold on;
    ha1(1).FaceColor=[255,255,51]./255;
    ha1(2).FaceColor=[255,127,0]./255;

    ha2=area(tmp.date,plotdata3'); hold on;
    ...
    tmp=dataALL.(varname);
    plot(tmp.date,tmp.profile(12,:)); hold on;
    ylabel('PAR @8m (W/m^2)');
    ylim([0 80]);
    ...
    ylim([8 10]);
    plot([timesteps(i) timesteps(i)],[4 6],'--','Color','r','LineWidth',2);
    hold on;

    ylabel('Secchi depth (m)');
    xlabel('date');
    title('(f) PAR @8m vs. Secchi','FontSize',fs+4);

    %hl=legend('OAS-par','OAS-secchi');
    %set(hl,'Location','northwest','FontSize',fs);
