...
Sori=load('E:\light_and_seagrass\model\seagrass\restart_08\results2.mat');
Sdredge=load('E:\light_and_seagrass\model\seagrass\restart_08_dredge\results2_dredge.mat');
...
t0=datenum(2022,3,10);
  ...
  %  cdata1(d<clip_depth)=NaN;
   ...
   % colormap('jet');
    ...
    %colorTitleHandle = get(cb,'Title');
    %set(colorTitleHandle ,'String',regexprep(varname,'_',' '),'color','k','fontsize',10);
    title('after 10 days','color','k','fontsize',10);hold on;
   %  dim = [0.4 0.91 0.3 0.05];
   %  str = ['Time: ', datestr(double(timesteps_WQ(i)),'yyyy/mm/dd HH:MM')];
   %  annotation('textbox',dim,'String',str,'FitBoxToText','on','LineStyle','none','FontSize',15);
    m_grid('box','fancy','tickdir','out');
   ...
   % colormap('jet');
    ...
    %colorTitleHandle = get(cb,'Title');
    %set(colorTitleHandle ,'String',regexprep(varname,'_',' '),'color','k','fontsize',10);
    title('after 20 days','color','k','fontsize',10);hold on;
   %  dim = [0.4 0.91 0.3 0.05];
   %  str = ['Time: ', datestr(double(timesteps_WQ(i)),'yyyy/mm/dd HH:MM')];
   %  annotation('textbox',dim,'String',str,'FitBoxToText','on','LineStyle','none','FontSize',15);
    m_grid('box','fancy','tickdir','out');
...
tt0=find(abs(timesteps-t0)==min(abs(timesteps-t0)));
   ...
   % colormap('jet');
    ...
    %colorTitleHandle = get(cb,'Title');
    %set(colorTitleHandle ,'String',regexprep(varname,'_',' '),'color','k','fontsize',10);
    title('after 30 days','color','k','fontsize',10);hold on;
   %  dim = [0.4 0.91 0.3 0.05];
   %  str = ['Time: ', datestr(double(timesteps_WQ(i)),'yyyy/mm/dd HH:MM')];
   %  annotation('textbox',dim,'String',str,'FitBoxToText','on','LineStyle','none','FontSize',15);
    m_grid('box','fancy','tickdir','out');
    hold on;
     ...
     img_name =['./compare_dredge_scenarios_seagrass_biomass_series.jpg'];
 saveas(gcf,img_name);