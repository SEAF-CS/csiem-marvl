...
for b=1:length(bands_str)
    ...
    Ldir(b,:)=output.Kwinana.(varname).profile(1,:);

    varname=upper(['WQ_DIAG_OAS_DIF_BAND',num2str(b)]);
    Ldif(b,:)=output.Kwinana.(varname).profile(1,:);
end
...
%monthlymean(1,:)=monthlymean(1,:)*1.8;
    ...
    %xlabel('Months');
	set(gca,'XTick',1:12,'XTickLabel',datestr(datearray,'mmm/yyyy'),'ylim',[0 400]);
    ylabel('\muW/cm^2/nm');
...
pngname=[outdir,'model_KwinanaShelf_Fixed_uW_2022_top_',scenario,'.png'];
...
%monthlymean(1,:)=monthlymean(1,:)*1.8;
    ...
    set(hl,'Location','eastoutside')
    %xlabel('Months');
	set(gca,'XTick',1:12,'XTickLabel',datestr(datearray,'mmm/yyyy'),'ylim',[0 1400]);
    ylabel('\muW/cm^2/nm');
    title('Light intensity at wavelengths - modelled')
...
pngname=[outdir, 'model_KwinanaShelf_Fixed_uW_2022_top_',scenario,'_12pm.png'];
print(gcf,'-dpng',pngname,'-r300');
