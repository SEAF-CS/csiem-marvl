...
for b=1:length(bands_str)
    ...
    Ldir(b,:)=output.Kwinana.(varname).profile(8,:);

    varname=upper(['WQ_DIAG_OAS_DIF_BAND',num2str(b)]);
    Ldif(b,:)=output.Kwinana.(varname).profile(8,:);
end
...
for ww=1:length(WL2)
   % tmp=spectrum.(wls{ww});
    ...
    for mm=1:length(datearray)
        ...
        inds=find(vec(:,1)==d2c(mm,1) & vec(:,2)==d2c(mm,2) & vec(:,4)==20);
...
%monthlymean(1,:)=monthlymean(1,:)*1.8;
    ...
    pngname='model_KwinanaShelf_Fixed_uW_2023_5m_12pm_half.png';
