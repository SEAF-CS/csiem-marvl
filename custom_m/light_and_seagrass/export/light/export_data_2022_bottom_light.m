clear; close all;
addpath(genpath('/Projects2/csiem/csiem-marvl/aed-marvl/'))
...
for ll=1:length(allvars)
    ...
    if any([strfind(loadname,'WQ_DIAG_OAS_DIR_BAND'), strfind(loadname,'WQ_DIAG_OAS_DIF_BAND')])
    disp(['loading ',loadname,' ...']);
    tic;

    rawData=ncread(ncfile(1).name, loadname); %load_AED_vars(ncfile,1,loadname,allvars);
    output.(loadname)=rawData(Bottcells,:);
    
    toc;
    end
end

save('extracted_bottom_light_2022_restart_08_nopollution70s_v2.mat','output','-mat','-v7.3');

else
    load extracted_bottom_light_2022_restart_08.mat;
end

%% sum to totol light

    for b=1:16
       varname1=upper(['WQ_DIAG_OAS_DIR_BAND',num2str(b)]);
       varname2=upper(['WQ_DIAG_OAS_DIF_BAND',num2str(b)]);
       varname=upper(['LIGHT_BAND',num2str(b)]);

       output2.(varname)=output.(varname1)+output.(varname2);
    end

    save('extracted_bottom_totalight_2022_restart_08_nopollution70s_v2.mat','output2','-mat','-v7.3');
% %% Export to NetCDF
% %layers=[24 17];
% 
