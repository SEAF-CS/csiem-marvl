...
WQfile=[inDir,'csiem_100_A_20220101_20221231_HD_waves_waveV1.nc'];
...
img_dir = '.\animation_waves_stress\';
...
t0=datenum('20220105 00:00','yyyymmdd HH:MM');
tt = find(abs(timesteps_WQ-t0)==min(abs(timesteps_WQ-t0)));
t1=datenum('20220120 00:00','yyyymmdd HH:MM');
...
for i=tt:plot_interval:tt1
   ...
   % caxis(trange);
    ...
    title('Current Speed','color','k','fontsize',10);hold on;
   ...
   % clear tdat;
    ...
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata1=tdat.TAUW; %sqrt(Wx.^2+Wy.^2);
...
%     clear tdat;
  ...
  %  caxis(trange);
    ...
    P1=get(cb,'Position');
    set(cb,'Position',[P1(1) P1(2)-0.05 P1(3) P1(4)]);
    cb.Label.String = '\tau_{wave} (N m^{-2})';
   ...
   % cbarrow;
    ...
    title('\tau_{wave}','color','k','fontsize',10);hold on;
    ...
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata1=tdat.TAUC; %sqrt(Wx.^2+Wy.^2);
   ...
   % caxis(trange);
    ...
    P1=get(cb,'Position');
    set(cb,'Position',[P1(1) P1(2)-0.05 P1(3) P1(4)]);
    cb.Label.String = '\tau_{current} (N m^{-2})';
...
hold on;
    ...
    title('\tau_{current}','color','k','fontsize',10);hold on;
