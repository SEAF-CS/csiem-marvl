...
HRfile=[inDirHR,'csiem_v1_C001_20220101_20221231_WQ_highRes_dredge_highRes.nc'];
WQfile=HRfile;
...
img_dir = '.\animation_dredge_highRes_patch_uncompressed\';
...
for i=tt:plot_interval:tt1
    ...
    names={'V_x';'V_y';'W';'D';'WQ_NCS_SS1';'cell_X';'cell_Y';'TEMP';'layerface_Z';'NL';'idx2'};
...
%     
    ...
    Fx = scatteredInterpolant(cell_X,cell_Y,double(tdat.V_x(bottom_cellsWQ)));
    zzzx=Fx(xxxs,yyys);zzzx(~cell_inds)=NaN;
    xxxu=zzzx(1:intv:end,1:intv:end);
    Fy = scatteredInterpolant(cell_X,cell_Y,double(tdat.V_y(bottom_cellsWQ)));
    zzzy=Fy(xxxs,yyys);zzzy(~cell_inds)=NaN;
...
%             set(colorTitleHandle,'String','T');
   ...
   % tdat = tfv_readnetcdf(WQfile,'timestep',i);
    ...
    title('Suspended Solids - curtain view (mg/L)',...
                'Units','Normalized',...
                'color','k','fontsize',14);
            caxis([0 20]);
