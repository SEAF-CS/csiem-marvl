...
img_dir = '.\animation_light_delmap_uncompressedV4\';
...
for i=tt:plot_interval:tt1
   ...
   % ind0=find(abs(timesteps_met-t0)==min(abs(timesteps_met-t0)));
    names={'V_x';'V_y';'D';'WQ_DIAG_TOT_PAR'};
...
%     hold on;
   ...
   % tdat2 = tfv_readnetcdf(wavefile,'timestep',ind0);
    ...
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata1=tdat2.WQ_DIAG_TOT_PAR(bottom_cells); %sqrt(Wx.^2+Wy.^2);
...
%     hold on;
    ...
    title('PAR - no dredge','color','k','fontsize',10);hold on;
    ...
    cdata12=tdat.WQ_DIAG_TOT_PAR(bottom_cells); %sqrt(Wx.^2+Wy.^2);
...
%     clear tdat;
   ...
   % cbarrow;
    ...
    title('PAR - dredge','color','k','fontsize',10);hold on;
    ...
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata13=cdata1-cdata12; %=tdat.WQ_DIAG_NCS_RESUS(bottom_cells); %sqrt(Wx.^2+Wy.^2);
...
%hold on;
    ...
    title('\DeltaPAR (no dredge - dredge)','color','k','fontsize',10);hold on;
