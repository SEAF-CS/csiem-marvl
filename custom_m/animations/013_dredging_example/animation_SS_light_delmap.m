...
WQfile=[inDir,'csiem_v1_C001_20220101_20221231_WQ_highRes_dredge_WQ.nc'];
wavefile=[inDir,'csiem_v1_C001_20220101_20221231_WQ_highRes_nodredge_WQ.nc'];
...
img_dir = ['.\animation_SS_light_delmap_uncompressed\'];
...
t1=datenum('20220401 00:00','yyyymmdd HH:MM');
...
for i=tt:plot_interval:tt1
   ...
   % ind0=find(abs(timesteps_met-t0)==min(abs(timesteps_met-t0)));
    names={'V_x';'V_y';'D';'WQ_NCS_SS1';'WQ_DIAG_TOT_PAR'};
    tdat = tfv_readnetcdf(WQfile,'names',names,'timestep',i);
    Wx=tdat.V_x(surf_cells);
    Wy=tdat.V_y(surf_cells);
...
%     hold on;
    ...
    title('Surface Current Speed','color','k','fontsize',10);hold on;
   ...
   % tdat2 = tfv_readnetcdf(wavefile,'timestep',ind0);
	tdat2 = tfv_readnetcdf(wavefile,'names',names,'timestep',ind0);
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata1=tdat2.WQ_NCS_SS1(surf_cells); %sqrt(Wx.^2+Wy.^2);
...
%     hold on;
    ...
    title('SS - no dredge','color','k','fontsize',10);hold on;
    ...
    cdata12=tdat.WQ_NCS_SS1(surf_cells); %sqrt(Wx.^2+Wy.^2);
    
    d=tdat.D;
    cdata12(d<clip_depth)=NaN;
%     
...
%     clear tdat;
   ...
   % F = scatteredInterpolant(cell_X,cell_Y,double(cdata12));
   % zzz=F(xxx,yyy);zzz(~cell_inds)=NaN;
   ...
   % cbarrow;
    ...
    title('SS - dredge','color','k','fontsize',10);hold on;
    ...
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata12=tdat2.WQ_DIAG_TOT_PAR(bottom_cells);
    cdata1 =tdat.WQ_DIAG_TOT_PAR(bottom_cells);
    cdata13=cdata12-cdata1; %=tdat.WQ_DIAG_NCS_RESUS(bottom_cells); %sqrt(Wx.^2+Wy.^2);
    
    d=tdat.D;
    cdata13(d<clip_depth)=NaN;
   ...
   hold on;
    ...
    P1=get(cb,'Position');
    set(cb,'Position',[P1(1) P1(2)-0.05 P1(3) P1(4)]);
    cb.Label.String = '\DeltaPAR (W/m^2)';
    colormap(ax4,summer)
...
%hold on;
    ...
    title('\DeltaPAR (dredge - no dredge)','color','k','fontsize',10);hold on;
