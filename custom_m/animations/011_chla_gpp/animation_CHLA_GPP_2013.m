...
inDirHR='W:\csiem\Model\TFV\Results_2013_B009\';

WQfile=[inDir,'csiem_v1_B009_20130101_20131231_WQ_WQ_noCan.nc'];
HRfile=WQfile;
...
img_dir = '.\animation_CHLA_GPP_2013V2\';
...
sim_name = [img_dir,'animation.avi','Uncompressed AVI'];
...
t0=datenum('20130205 12:00','yyyymmdd HH:MM');
tt = find(abs(timesteps_WQ-t0)==min(abs(timesteps_WQ-t0)));
t1=datenum('20130220 12:00','yyyymmdd HH:MM');
...
for i=tt:plot_interval:tt1
    ...
    names={'WQ_DIAG_PHY_TCHLA'; 'WQ_DIAG_PHY_GPP';'cell_X';'cell_Y';...
        'V_x';'V_y';'layerface_Z';'idx2';'NL';'D'};
    tdat = tfv_readnetcdf(HRfile,'names',names,'timestep',ind0);
    cdata1=tdat.WQ_DIAG_PHY_TCHLA(surf_cellsWQ);
    d=tdat.D;
...
%     colormap(cmap);
  ...
  %  hold on;
    ...
    title('Surface TCHLA (\mug/L)','color','k','fontsize',14);hold on;
   ...
   % tdat = tfv_readnetcdf(WQfile,'timestep',i);
    plotdata=tdat.WQ_DIAG_PHY_TCHLA;
    ...
    title('TCHLA - curtain view (\mug/L)',...
                'Units','Normalized',...
                'color','k','fontsize',14);
            caxis([0 6]);
            colormap(cmap);
...
%             set(colorTitleHandle,'String','T');
   ...
   % tdat = tfv_readnetcdf(WQfile,'timestep',i);
    ...
    title('GPP - curtain view (mmol C/m^3/d)',...
                'Units','Normalized',...
                'color','k','fontsize',14);
            caxis([0 10]);
            colormap(cmap);
