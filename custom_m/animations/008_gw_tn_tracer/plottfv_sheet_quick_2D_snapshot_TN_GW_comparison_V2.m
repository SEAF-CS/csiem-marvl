...
img_dir = '.\animation_TN_GW_v3_July_surface\';
...
t1=datenum('20130801 00:00','yyyymmdd HH:MM');
...
for i=tt:plot_interval:tt1
   ...
   % ind0=find(abs(timesteps_met-t0)==min(abs(timesteps_met-t0)));
    names={'WQ_DIAG_TOT_TN';'D';'WQ_TRC_TR2'};
    tdat1 = tfv_readnetcdf(WQfile,'names',names,'timestep',i);
   % Wx=tdat.W10_x;
   % Wy=tdat.W10_y;
    cdata1=tdat1.WQ_DIAG_TOT_TN(surf_cells)*14; %sqrt(Wx.^2+Wy.^2);
    d=tdat1.D;
   ...
   % hold on;
    ...
    title('Surface TN - with GW','color','k','fontsize',10);hold on;
    ...
    tdat = tfv_readnetcdf(WQfile2,'names',names,'timestep',ind0);
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata2=tdat.WQ_DIAG_TOT_TN(surf_cells)*14; %sqrt(Wx.^2+Wy.^2);
   ...
   % cmap(1,:)=[0.8 0.8 0.8];
    ...
    title('Surface TN - no GW','color','k','fontsize',10);hold on;
  ...
  %      hold on;
   ...
   % tdat = tfv_readnetcdf(WQfile,'timestep',i);
    %Wx=tdat.W10_x;
    %Wy=tdat.W10_y;
    cdata4=tdat1.WQ_TRC_TR2(surf_cells)*100; %sqrt(Wx.^2+Wy.^2);
...
hold on;
    ...
    title('Surface GW Tracer','color','k','fontsize',10);hold on;
