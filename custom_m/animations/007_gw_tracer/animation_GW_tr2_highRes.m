...
WQfile=[inDir,'csiem_v1_C001_20220101_20221231_WQ_highRes_WQ.nc'];
HRfile=WQfile;
...
def.xlim = [0 12.53];% xlim in KM
def.xticks = [0    6.2   12.5]; %[0:5:45];
def.xticklabels = {'offshore','Garden Is.','nearshore'};
...
polyline_file='../GIS/Transec_line_LL_GW.shp';
...
img_dir = '.\animation_GW_tr2_HighRes\';
...
t1=datenum('20220331 00:00','yyyymmdd HH:MM');
...
for i=tt:plot_interval:tt1
    ...
    cdata1=tdat.WQ_TRC_TR2(bottom_cellsWQ)*100;
    d=tdat.D;
   ...
   % hold on;
    ...
    title('Bottom Groundwater Tracer (%)','color','k','fontsize',14);hold on;
   ...
   % tdat = tfv_readnetcdf(WQfile,'timestep',i);
    plotdata=tdat.WQ_NIT_AMM*14/1000*1000;
    ...
    title('Ammonium - curtain view (\mug N/L)',...
                'Units','Normalized',...
                'color','k','fontsize',14);
            caxis([0 40]);
            colormap(cmap);
...
%             set(colorTitleHandle,'String','T');
   ...
   % tdat = tfv_readnetcdf(WQfile,'timestep',i);
    ...
    title('Groundwater tracer - curtain view (%)',...
                'Units','Normalized',...
                'color','k','fontsize',14);
            caxis([0 0.5]);
            colormap(cmap);
