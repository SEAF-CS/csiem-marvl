...
HRfile=[inDirHR,'csiem_100_A_20130101_20130601_WQ_009_waves_nutirent_trc_GW_HighRes_hires.nc'];
...
def.xlim = [0 27.3];% xlim in KM
def.xticks = [0    6.36   13.06   20.12   27.0]; %[0:5:45];
def.xticklabels = {'offshore','Cape Peron','central CS','Coogee','Fremantle'};
...
polyline_file='../GIS/New_Curtain_line_LL_Dist.shp';
...
for i=tt:plot_interval:tt1
    ...
    cdata1=tdat.WQ_OXY_OXY(bottom_cellsHR)*32/1000;
    d=tdat.D;
   ...
   % F1 = fill(X1,Y1,[146, 116, 91]./255); %140,81,10
    ...
    title('Bottom Oxygen (mg/L)','color','k','fontsize',14);hold on;
...
%             set(colorTitleHandle,'String','T');
   set(gca,'box','on','LineWidth',1.0,'Layer','top');
    ...
    tdat = tfv_readnetcdf(WQfile,'timestep',i);
    plotdata=tdat.WQ_OXY_OXY*32/1000;
    ...
    title('Dissolved Oxygen - curtain view (mg/L)',...
                'Units','Normalized',...
                'color','k','fontsize',14);
            caxis([4 8]);
