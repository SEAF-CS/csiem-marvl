...
img_dir = '.\animation_AED_light_newMZV3\';
...
for i=tt:plot_interval:tt1
   ...
   % ind0=find(abs(timesteps_met-t0)==min(abs(timesteps_met-t0)));
    names={'V_x';'V_y';'D';'WQ_DIAG_TOT_TURBIDITY';'WQ_DIAG_TOT_PAR'};
...
%     
  ...
  %  hq0=quiver(xxxx,yyyy,xxxu,xxxv);
   ...
   % clear tdat;
    ...
    P1=get(cb,'Position');
    set(cb,'Position',[P1(1) P1(2)-0.05 P1(3) P1(4)]);
    cb.Label.String = 'Solar Radiation (W m^{-2})';
    hold on;
    m_plot(shp2.X,shp2.Y,'Color',[0.2 0.2 0.2]);
    hold on;
    %cbarrow;
    ...
    title('Solar Radiation','color','k','fontsize',10);hold on;
