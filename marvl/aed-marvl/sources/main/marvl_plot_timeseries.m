...
for var = config.start_plot_ID:config.end_plot_ID
    ...
    for site = sites
        ...
        isConv=0;
        MatchedData = cell(1,length(config.plotdepth));
        ...
        for j = 1:length(sss)
            if isfield(fdata.(sitenames{sss(j)}),loadname)
                ...
                if ~isempty(xdata_t)
                    ...
                    [mface,mcolor,~,agencyname] = marvl_sort_agency_information(agency);   % 2026-09-08: match new 4-output signature (was 3-output; passing fdata as numPoints crashed isfinite for dynamic-sizing agencies)
