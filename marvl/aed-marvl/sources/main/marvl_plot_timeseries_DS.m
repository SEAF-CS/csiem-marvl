function marvl_plot_timeseries_DS(MARVLs,style)
%**************************************************************************
%
...
for var = config.start_plot_ID:config.end_plot_ID
    ...
    for site = sites
        ...
        pngName = regexprep(finalname_p,'\.eps','.png');
        jpgName = regexprep(finalname_p,'\.eps','.jpg');
        set(gcf,'ToolBar','none');
        
        if isfield(config,'filetype')
            if strcmpi(config.filetype,'png')
                % Feb 12, 2026: Add robust export fallbacks to avoid batch
                % termination when print/export intermittently fails.
                try
                    print(gcf,'-dpng',pngName,'-r300');
                catch MEprint
                    warning('PNG export failed with print (%s). Retrying with software renderer.',MEprint.message);
                    try
                        opengl software;
                        drawnow;
                        exportgraphics(gcf,pngName,'Resolution',300);
                    catch MEexport
                        warning('PNG export failed with exportgraphics (%s). Falling back to saveas.',MEexport.message);
                        try
                            saveas(gcf,pngName);
                        catch MEsave
                            warning('PNG export failed with saveas (%s). Stopping run to preserve output integrity.',MEsave.message);
                            error('MARVL:ExportFailed','Failed to export figure after 3 attempts: %s',pngName);
                        end
                    end
                end
            else
                ...
                try
                    exportgraphics(gcf,jpgName,'Resolution',300)
                catch MEjpg
                    warning('JPG export failed with exportgraphics (%s). Falling back to print.',MEjpg.message);
                    try
                        opengl software;
                        drawnow;
                        print(gcf,'-djpeg',jpgName,'-r300');
                    catch MEjpg2
                        warning('JPG export failed with print (%s). Falling back to saveas.',MEjpg2.message);
                        try
                            saveas(gcf,jpgName);
                        catch MEsave
                            warning('JPG export failed with saveas (%s). Stopping run to preserve output integrity.',MEsave.message);
                            error('MARVL:ExportFailed','Failed to export figure after 3 attempts: %s',jpgName);
                        end
                    end
                end
            end
