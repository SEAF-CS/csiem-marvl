%% stats18 chunk vars 15-20 (per-chunk errormatrix; single-model OOM'd at var 22 unchunked)
%% STATS PASS (2026-09-10): single-model csiem1.8.0 as ncfile(1) so the
%% errorMatrix (computed for mod==1 only) scores the 1.8 run. Outputs to
%% 2021A_stats18/ to keep the 1.7-vs-1.8 comparison figures separate.
...
master.varname = { ...
    ...
    'WQ_DIAG_TOT_TP','Total Phosphorus'; ...
     'WQ_PHS_FRP','Phosphate'; ...
     'WQ_OGM_PON','PON'; ...
     'WQ_OGM_POC','POC'; ...
    'WQ_DIAG_TOT_PAR','PAR'; ...
            'H','Water Level'; ...
    'WQ_DIAG_TOT_EXTC','Light Extinction'; ...
    'WQ_DIAG_PHY_MPB_BEN','MPB'; ...
    'WQ_NCS_SS1','Suspended Solids 1'; ...
    'WQ_NCS_SS2','Suspended Solids 2'; ...
    'WQ_DIAG_PHY_MPB_RES','MPB resuspension'; ...
    'WQ_DIAG_OGM_PON_RES','PON RES ()'; ...
     'WQ_DIAG_OGM_PON_SWI','PON SWI ()'; ...
     'WQ_DIAG_OGM_OM_SED_FRAC','OM FRAC ()'; ...
...
% Models
%master.ncfile(1).name = '/Projects2/csiem/model/csiem_model_tfvaed_1.6/outputs/2022B/csiem_B009_20211101_20221231_WQ_WQ.nc';
%master.ncfile(1).legend = 'csiem1.6';
%master.ncfile(1).tag = 'TFV';

%master.ncfile(1).name = 'W:\WAMSI\1.7\SH-20251123-1.7.0\2021B-20260131010652\results\csiem_B010_20201101_20211231_WQ_WQ.nc';   % B-mesh 1.7 baseline (was 2021A A-mesh; B mesh throughout, Matt 2026-09-07)  % dropped for stats pass: errorMatrix scores ncfile(1) only
%master.ncfile(1).legend = 'csiem1.7.0';
%master.ncfile(1).tag = 'TFV';
master.ncfile(1).name = 'Q:\SEAF-CS\V1.8\MODEL\csiem_model_tfvaed_1.8\output_archive\1.8.0\2022B\csiem_B010_20211101_20221231_WQ_WQ.nc';
master.ncfile(1).legend = 'csiem1.8.0';
master.ncfile(1).tag = 'TFV';

% field data
master.add_fielddata = 1;
master.fielddata_folder = 'G:\CSIEM\V1.7\DATA\csiem-data\data-warehouse\mat\agency\';
master.fielddata_files = {'csiem_AIMS_public','csiem_BMT_public','csiem_BOM_public','csiem_CSIRO_public','csiem_CSMC_public','csiem_DEP_public',...
   'csiem_DOT_public','csiem_DWER_public','csiem_DPIRD_public','csiem_ESA_public','csiem_FPA_public','csiem_WCWA_public','csiem_WAMSI_public',...
   'csiem_IMOS_public','csiem_UWA_public'};
%master.fielddata_folder = '../../data/20250925/';
%master.fielddata_files = {'csiem_AIMS_public','csiem_BMT_public','csiem_CSIRO_public','csiem_CSMC_public','csiem_DEP_public',...
%   'csiem_DOT_public','csiem_DWER_public','csiem_DPIRD_public','csiem_ESA_public','csiem_FPA_public','csiem_WCWA_public','csiem_WAMSI_public',...
...
timeseries.start_plot_ID = 15;
timeseries.end_plot_ID = 20;   % = numel(varname); 31 overran the list and crashed after the last variable
...
timeseries.polygon_file = 'Q:\SEAF-CS\V1.8\MARVL\csiem-marvl\gis\Zones\MLAU_Zones_v3_ll.shp';   % absolute: relative ../../gis only resolves from marvl/configs/
...
timeseries.outputdirectory = '../../outputs/008_1.8_comparison/2022B_stats18/RAW/';
timeseries.htmloutput = '../../outputs/008_1.8_comparison/2021A/HTML/';
timeseries.ErrFilename = '../../outputs/008_1.8_comparison/2022B_stats18/errormatrix_chunk15_20.mat';
...
axis_map = {
    'TEMP',                 [14    28];     % °C
    'SAL',                  [30    38];     % PSU
    'WQ_OXY_OXY',           [2.0   11];     % mg/L
    'WQ_DIAG_TOT_TN',       [0     2];      % mg/L
    'WQ_NIT_AMM',           [0     0.09];    % mg/L
    'WQ_NIT_NIT',           [0     0.09];   % mg/L
    'WQ_DIAG_TOT_TP',       [0     0.05];   % mg/L
    'WQ_DIAG_PHY_TCHLA',    [0     7];     % µg/L
    'WQ_DIAG_TOT_TOC',      [0     5];      % mg/L
    'WQ_DIAG_TOT_TSS',      [0     40];     % mg/L
    ...
    'WQ_DIAG_TOT_EXTC',     [0     2.0];      % m^-1
    'RHOW',                 [1000  1030];   % kg/m^3
    'LW_RAD',               [200   450];    % W/m^2
    'WQ_PHS_FRP',           [0     0.04];   % mg/L (PO4) — adjust to your dataset
    'WQ_DIAG_TOT_TURBIDITY',[0     17];     % NTU
