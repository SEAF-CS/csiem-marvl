# =====================================================================================
# FLOOD-TIDE CLIP  (Cockburn Sound PYCNOCLINE, 22 Aug 1991, hourly, 37 frames)
# Short, smooth clip of the dense pulse advancing into the north basin as the tide floods
# (cf. thesis Fig 6.22, 22 Aug 05:00). Variant of render_hq_1991_orbit.R with:
#   - DYNAMIC water level: the dielectric sea-surface slab rises/falls per frame from
#     data/pyc_1991_flood/water_level.csv (3h-smoothed CS-mean free surface; tames the seiche
#     so it reads as a tidal rise/fall). elev(m) -> world y via /ZSCALE, matching the mesh.
#   - GENTLE near-static camera (subtle push-in + small drift) so the eye stays on the
#     water+pulse dynamics, not camera motion.
#   - pycnocline: diffuse violet #762a83 ; render: NEE + bright wrap + 256/sobol_blue + denoise.
# Reads data/pyc_1991_flood ; writes images/hq_1991_flood.  Resumable (existing PNGs skipped).
# =====================================================================================
...
SDIR  <- "data/pyc_1991_flood"
WLCSV <- "data/pyc_1991_flood/water_level.csv"
OUT   <- "images/hq_1991_flood"; dir.create(OUT, showWarnings = FALSE)
ZSCALE <- 0.4
SAMP  <- 128; W <- 880; H <- 760          # "fast" budget (~16 min/frame on this 4-core box)
# gentle near-static camera (north-facing hero, small drift + slow push-in over the clip)
THETA0 <- 18; THETA1 <- 10; PHI0 <- 40; PHI1 <- 43; ZOOM0 <- 0.60; ZOOM1 <- 0.72
...
N <- length(tifs)
wl <- read.csv(WLCSV)                            # frame,time,elev_m  (per-frame sea-surface level)
stopifnot(nrow(wl) == N)
cat(sprintf("terrain %dx%d | frames %d | samples %d | FLOOD CLIP (dynamic water %.3f..%.3f m)\n",
            nr, nc, N, SAMP, min(wl$elev_m), max(wl$elev_m)))
...
cam_for <- function(i) {                         # gentle move: 0 -> 1 across the clip
...
for (i in seq_len(N)) {
  ...
  wy    <- wl$elev_m[i] / ZSCALE                 # sea-surface elevation (m) -> world y (mesh scale)
  ...
  cat(sprintf("[%2d/%d] %s  wlev %+.3f m  %.0fs  (elapsed %.1f min, %d rendered)\n",
              i, N, basename(outpng), wl$elev_m[i], el, tot, done))
}
rgl::close3d()
cat("FLOOD CLIP DONE\n")
