# Time series of the O2 = 6 mg/L isosurface elevation, 13-23 Jan 2024 (4-hourly).
# Vectorised crossing detection; writes a coarse DEM + one interface GeoTIFF per
# timestep on the SAME coarse grid (so R renders fast and perfectly aligned).
...
ODIR= r"G:/CSIEM/1.8.0/csiem-marvl/rayshader/data/oxy6_series"
...
sel = np.where((tt >= dt.datetime(2024,1,13,0)) & (tt <= dt.datetime(2024,1,23,23)))[0]
...
for n, ti in enumerate(sel):
    ...
    print(f"frame {n:02d} {str(times[ti])[:13]}  cols<6={len(ucol):4d}  gridcells={int(np.isfinite(grid).sum()):5d}")
