# =====================================================================================
# FULL HQ PATH-TRACED SERIES  (Cockburn Sound PYCNOCLINE, 1991 winter STORM, Aug 11-26, 61 frames)
# Port of render_hq_jan2024_orbit.R (continuous slow orbit) with the isosurface field swapped
# from the oxycline (O2=5.5, red) to the PYCNOCLINE (sigma-t=25.5, violet). Same locked HQ config:
...
# Reads data/pyc_1991 ; writes images/hq_1991.  Resumable (existing PNGs skipped).
# =====================================================================================
...
SDIR  <- "data/pyc_1991"
OUT   <- "images/hq_1991"; dir.create(OUT, showWarnings = FALSE)
ZSCALE <- 0.4
SAMP  <- 256; W <- 1100; H <- 950
# continuous slow orbit, biased north-facing (theta ~15 is the 1991 hero). Sweep is gentle so the
# storm de-/re-stratification (the surface collapsing then rebuilding) reads, not the camera motion.
...
N <- length(tifs)
cat(sprintf("terrain %dx%d | frames %d | samples %d | CONTINUOUS ORBIT (pycnocline)\n", nr, nc, N, SAMP))
