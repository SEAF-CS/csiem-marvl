...
for (i in seq_len(nrow(lm))){ r<-lm$row[i]; c<-lm$col[i]; cc<-cols[[i]]
...
plot_3d(tex,elmat,zscale=ZSCALE,fov=0,theta=0,phi=90,windowsize=c(560,1100),zoom=0.95,water=FALSE); Sys.sleep(0.2)
render_camera(theta=0,phi=90,zoom=0.95,fov=0)
render_snapshot("images/landmarks_plan3d.png",clear=FALSE); rgl::close3d(); cat("done\n")
