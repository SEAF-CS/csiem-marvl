...
elmat <- raster_to_matrix(raster("data/dem_coarse_fixed.tif"))   # rtm = CORRECT terrain (confirmed)
...
salm <- raster_to_matrix(raster("data/sal_bottom/sal_059.tif"))  # salt_rtm, same orientation as terrain
...
ov[,,4]<-ifelse(is.na(salm),0,0.85)
pc <- list(c(1,0,0),c(0,0,1),c(0,1,0),c(1,0,1),c(1,1,0)); d<-8
for (i in seq_len(nrow(lm))){ r<-lm$col[i]; c<-lm$row[i]; cc<-pc[[i]]   # rtm[C,R]=array[R,C]: row=col, col=row
...
tex<-add_overlay(add_shadow(sphere_shade(elmat,zscale=ZSCALE,texture="imhof1"),ms,0.5),ov,alphalayer=0.88)
...
render_snapshot("images/verify_rtm_plan.png",clear=FALSE);rgl::close3d();cat("done\n")
