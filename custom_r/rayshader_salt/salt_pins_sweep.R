...
pc <- list(c(1,0,0),c(0,0,1),c(0,1,0),c(1,0,1),c(1,1,0))   # Garden Fremantle(N) Woodman(E) CapePeron(S) Carnac(W)
d<-8
for (i in seq_len(nrow(lm))){ r<-lm$col[i]; c<-lm$row[i]; cc<-pc[[i]]   # swap+none (tracks salt)
...
tex<-add_overlay(add_shadow(add_shadow(sphere_shade(elmat,zscale=ZSCALE,texture="imhof1"),ms,0.5),ma,0),ov,alphalayer=0.95)
dir.create("images/sps",showWarnings=FALSE)
for (th in seq(0,315,by=45)){
  plot_3d(tex,elmat,zscale=ZSCALE,fov=0,theta=th,phi=45,windowsize=c(1000,850),zoom=0.6,water=FALSE); Sys.sleep(0.12)
  render_camera(theta=th,phi=45,zoom=0.6,fov=0)
  render_snapshot(sprintf("images/sps/th%03d.png",th),clear=FALSE); rgl::clear3d(); cat(th,"\n") }
rgl::close3d()
