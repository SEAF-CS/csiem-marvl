...
elmat <- t(raster_to_matrix(raster("data/dem_coarse_fixed.tif")))   # = north-up array, 847x419
...
salm <- t(raster_to_matrix(raster("data/sal_bottom/sal_059.tif")))  # transpose to match
...
d<-8
for (i in seq_len(nrow(lm))){ r<-lm$row[i]; c<-lm$col[i]; cc<-pc[[i]]   # PLAIN array coords (elmat=array)
...
tex<-add_overlay(add_shadow(sphere_shade(elmat,zscale=ZSCALE,texture="imhof1"),ms,0.5),ov,alphalayer=0.9)
...
render_snapshot("images/final_test_plan.png",clear=FALSE);rgl::close3d();cat("done\n")
