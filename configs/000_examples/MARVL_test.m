...
%
%--------------------------------------------------------------------------
master.modules = {'timeseries';'transect';'sheet';'curtain'; };
...
master.varname = {'SAL','Salinity';'TEMP','Temperature'; };
   
master.add_human = 1; % option to use user-define names, if 0 use AED names

% Models
master.ncfile(1).name = '/Projects2/csiem/Model/TFV/csiem_model_tfvaed_1.1/outputs/results/csiem_v1_A001_20220101_20221231_WQ_lowRes.nc';
master.ncfile(1).legend = 'csiem v1.0';
master.ncfile(1).tag = 'TFV';


% field data
master.add_fielddata = 1;
master.fielddata_matfile = '/Projects2/csiem/csiem-marvl-clean/data/cockburn.mat';
master.fielddata = 'cockburn'; 

% general plotting features
master.font = 'Times New Roman'; 
master.fontsize   = 9;
master.xlabelsize = 10;
master.ylabelsize = 10;
master.titlesize  = 10;
master.legendsize = 6;
...
timeseries.polygon_file = '/Projects2/csiem/csiem-marvl-clean/gis/MLAU_Zones_v3_ll.shp';
timeseries.plotAllsites = 1;
if timeseries.plotAllsites == 0;timeseries.plotsite = [6 10 29];end
...
timeseries.skills = [1,1,1,1,1,1,1,];
% r: regression coefficient (0-1)
% BIAS: bias relative to mean observation (%)
 % MAE: mean absolute error
 % RMSE: root mean square error
 % NMAE: MAE normalized to mean observation
 % NRMS: RMSE normalized to mean observation
 % MEF: model efficienty, Nash-Sutcliffe Efficiency
    
timeseries.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/timeseries_2022/RAW/';
timeseries.htmloutput = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/timeseries_2022/HTML/';
timeseries.ErrFilename = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/timeseries_2022/errormatrix.mat';
...
timeseries.datearray = datenum(2022,1:3:13,1);
...
% ylim
 for vvvv=1:size(MARVLs.master.varname,1);timeseries.cAxis(vvvv).value = [ ];end
...
transect.polygon_file = '/Projects2/csiem/csiem-marvl-clean/gis/New_Curtain_line_LL_Dist.shp';
...
% ___
transect.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/transect_2022/RAW/';
transect.htmloutput = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/transect_2022/HTML/';
...
for i=1:7;transect.pdates(i).value = [datenum(2022,04,1+(i-1)*5) datenum(2022,04,1+i*5)];end
...
% ylim
transect.cAxis(1).value = [34 37]; %SAL
...
% -------------------------------------------------------------------------
sheet.cAxis(2).value = [21 28]; %[35.5 36.5];
...
sheet.coastline_file = '/Projects2/csiem/csiem-marvl-clean/gis/Boundary.shp';  % 1/0: add/no coast line
...
%if strcmpi(sheet.plottype,'movie')
    ...
    sheet.datearray = [datenum(2022,01,15) datenum(2022,01,20)];
    sheet.dateformat = 'mm/yyyy';
    sheet.plot_interval = 1;
%elseif strcmpi(sheet.plottype,'figure')
    sheet.datearray = [datenum(2022,01,15) datenum(2022,01,16)];
    sheet.resolution = [1024,768]; % frame rosolution
    sheet.colormap = 'turbo'; % colormap options, see Matlab manual
%else
...
sheet.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/sheet_TEMP/';
...
curtain.cAxis(2).value =[21 27];    %[35.5 36.5];

curtain.polyline = '/Projects2/csiem/csiem-marvl-clean/gis/New_Curtain_line_LL_Dist.shp';
...
curtain.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/curtain_TEMP/';
...
%  The is the configuration file for the marvl_plot_transect.m function.
% -------------------------------------------------------------------------
transectSA.thevars = { 'WQ_PHY_GRN';'WQ_PHY_DINO';'WQ_PHY_MDIAT';'WQ_PHY_CRYPT';'WQ_PHY_BGA';};

transectSA.thelabels = {'Green Algae';'Dino';'Diatom';'Crypt';'Cyano';};
 
transectSA.thevars_conv = 12./[50 50 50 50 50];

transectSA.varname = {'WQ_DIAG_PHY_TCHLA','TChla (/mug/L)';};
   
    
transectSA.add_human = 1;

transectSA.polygon_file = '/Projects2/csiem/csiem-marvl-clean/gis/New_Curtain_line_LL_Dist.shp';
% Add field data to figure
...
% ___
transectSA.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/transectSA/RAW/';
transectSA.htmloutput = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/transectSA/HTML/';
...
for i=1:12;transectSA.pdates(1).value = [datenum(2013,i,01) datenum(2013,i+1,01)];i=i+1;end
...
% -------------------------------------------------------------------------
for vvvv=1:length(MARVLs.master.varname);profile.cAxis(vvvv).value = [ ];end
...
profile.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/profile/RAW/';
profile.htmloutput = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/profile/HTML/';
...
transectExc.polygon_file = '/Projects2/csiem/csiem-marvl-clean/gis/New_Curtain_line_LL_Dist.shp';
% Add field data to figure
...
transectExc.thresh(14).legend = {'%time > 1.2 mg/L','%time > 1.6 mg/L'};
    
% ___
transectExc.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/transect_exceedance/RAW/';
transectExc.htmloutput = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/transect_exceedance/HTML/';
...
% ylim
for vvvv=1:size(MARVLs.master.varname,1);transectExc.cAxis(vvvv).value = [ ];end
...
boxchartConf.polygon_file = '/Projects2/csiem/csiem-marvl-clean/gis/MLAU_Zones_v2_ll.shp';
boxchartConf.cattt=["CS02","CS03","CS04","CS05","CS06","CS07","CS08","CS09","CS10","CS11", "CS12","CS13","CS14","CS15","CS16","CS17","CS18","CS19","CS20","CS21","CS22","CS23","CS24","SC1","sc2","sc3","sc4"];
    

% time period to extract data
boxchartConf.datearray = [datenum(2013,4,1) datenum(2013,5,1)];
% ylim
for vvvv=1:size(MARVLs.master.varname,1);boxchartConf.cAxis(vvvv).value = [0 1];end
...
boxchartConf.outputdirectory = '/Projects2/csiem/csiem-marvl-clean/outputs_BB/boxchart2/RAW/';
...
NBudget.Pool_3D={'WQ_OGM_DON','DON';'WQ_OGM_DONR','DONR';'WQ_OGM_PON','PON';'WQ_NIT_NIT','NIT';'WQ_NIT_AMM','AMM';};
...
NBudget.BGC_3D={'WQ_DIAG_NIT_ANAMMOX','ANAMMOX';'WQ_DIAG_NIT_DENIT','DENIT';'WQ_DIAG_NIT_DNRA','DNRA';'WQ_DIAG_NIT_NITRIF','NITRIF';};


NBudget.BGC_3D_factors=[1 1 1 1]*14/1e9;

% 2D vars name for BGC process
NBudget.BGC_2D={'WQ_DIAG_NIT_SED_AMM','SED-AMM';'WQ_DIAG_NIT_SED_NIT','SED-NIT';};
...
if NBudget.plotAllsites == 0;NBudget.plotsite = [5];end
    
%end

NBudget.inputdirectory = 'E:/database/AED-MARVl-v0.3/Examples/Cockburn/Data/nutrient_budget/matfiles_kw001/';
