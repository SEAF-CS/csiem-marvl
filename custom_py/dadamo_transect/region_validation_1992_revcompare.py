...
Runs the region-validation extraction twice over the SAME casts (1992 comparison window
22-Feb -> 3-Apr, where both sims overlap; rev ends 3-Apr):
...
Then plots per-region surface/bottom S & T bias, baseline vs rev, and prints the headline change.
Obs are identical between runs; only the paired model differs. Baseline CSV kept.
"""
...
df_base.to_csv(os.path.join(DIR, 'region_validation_1992.csv'), index=False)

print('\n=== REV 1992 extraction ===')
core.FV_RUNS[1] = ('1992-02-22', '1992-04-03', REV_NC)   # repoint the 1992 entry at the rev NC
core._FV_CACHE.clear()
df_rev = core.run(WIN)
df_rev.to_csv(os.path.join(DIR, 'region_validation_1992_rev.csv'), index=False)
...
# ---- per-region bias table + grouped-bar comparison ----
...
for ax, (lvl, v) in zip(axes.ravel(), [('surf', 'S'), ('bot', 'S'), ('surf', 'T'), ('bot', 'T')]):
    ...
    for k, (tag, d, col) in enumerate([('baseline', df_base, '#888888'), ('rev', df_rev, '#1f77b4')]):
        ...
        bars = ax.bar(x + (k - 0.5) * w, vals, w, color=col, label=tag)
...
p = os.path.join(DIR, 'region_validation_1992_revcompare.png')
fig.savefig(p, dpi=150); plt.close(fig)
pd.DataFrame(recs).to_csv(os.path.join(DIR, 'region_validation_1992_revcompare.csv'), index=False)
print('\nwrote', p)
