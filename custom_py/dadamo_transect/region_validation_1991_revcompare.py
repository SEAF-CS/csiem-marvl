...
Same casts, two paired models:
  - baseline : output_archive/1.7.0/1991_aug/csiem_B010_19910720_19910831.nc
  - rev      : output_archive/1.7.0/1991_aug_rev/csiem_B010_19910720_19910831_rev.nc
...
Per-region surface/bottom S & T bias, baseline vs rev, + headline change. Baselines kept.
"""
...
REV_NC = r'S:/Matt_Working/csiem/output_archive/1.7.0/1991_aug_rev/csiem_B010_19910720_19910831_rev.nc'
REGIONS = ['OA', 'CS', 'N', 'NW', 'W', 'SW', 'S']

print('=== BASELINE 1991 extraction ===')
df_base = core.run(WIN)
df_base.to_csv(os.path.join(DIR, 'region_validation_1991_base.csv'), index=False)

print('\n=== REV 1991 extraction ===')
core.FV_RUNS[0] = ('1991-07-20', '1991-08-31', REV_NC)   # repoint the 1991 entry at the rev NC
core._FV_CACHE.clear()
df_rev = core.run(WIN)
df_rev.to_csv(os.path.join(DIR, 'region_validation_1991_rev.csv'), index=False)
...
p = os.path.join(DIR, 'region_validation_1991_revcompare.png')
fig.savefig(p, dpi=150); plt.close(fig)
pd.DataFrame(recs).to_csv(os.path.join(DIR, 'region_validation_1991_revcompare.csv'), index=False)
print('\nwrote', p)
