...
OUT_PNG_DIR = Path(r'Q:/SEAF-CS/V1.8/MARVL/csiem-marvl/custom_py/validation/years/1992/outputs/transectA')  # REV
...
# === Model: open 1992 NC, sample fixed union stations ========================
# (density derived per-profile from S,T below -- never build a full-field RHOW:
#  on the hourly NC that is a 5.7 GiB alloc -> OOM)
...
def model_profiles_at(model_date):
    ...
    for stn in MODEL_STATIONS:
        ...
        try:
            ...
            den = eos80_potential_density(sal, temp) - 1000.0   # sigma_t to match field (from S,T)
