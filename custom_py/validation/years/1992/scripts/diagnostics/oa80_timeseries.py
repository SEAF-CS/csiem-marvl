"""OA80 surface & bottom time series (1992): model T, S, rho vs SMCWS field casts. T panel also
shows the NAR (Swan) inflow-T BC. Ported from 1991. 1992 rev uses the RAW SCE inflow BC
(TEMP flat 21.6 C, SAL flat 27.16 psu placeholders - not the Narrows climatology)."""
...
OUT = r'Q:/SEAF-CS/V1.8/MARVL/csiem-marvl/custom_py/validation/years/1992/outputs/TimeSeries/oa80_timeseries_TSrho_1992_rev.png'
import os as _os; _os.makedirs(_os.path.dirname(OUT), exist_ok=True)
SITE = 'OA80'; LONLAT = (115.7013, -32.1313); YEAR = 1992; SIM0 = pd.Timestamp('1992-02-22')
IC_WEST = dict(T=20.9, S=35.8)
...
cs = inv[(inv.station == SITE) & (inv.year == YEAR)].copy()
...
print(f'{SITE} {YEAR} field casts in window: {len(fdt)}  ({fdt.min():%d-%b} .. {last:%d-%b %H:%M})')

# NAR inflow T (RAW CSV: flat 21.6 C placeholder for 1992)
nar = pd.read_csv(f'{SCE}/NAR_Inflow_19700101_19941231.csv'); nar['t'] = pd.to_datetime(nar['Date'], format='%d/%m/%Y')
narT = nar.set_index('t')['TEMP']; narT = narT[(narT.index >= SIM0 - pd.Timedelta('2d')) & (narT.index <= last + pd.Timedelta('2d'))]

def load_flow(tag):
    f = pd.read_csv(rf'{SCE}/{tag}_Inflow_19700101_19941231.csv'); f['t'] = pd.to_datetime(f['Date'], format='%d/%m/%Y'); return f.set_index('t')['Flow']
...
wci = int(np.argmin(((mX - LONLAT[0]) * np.cos(np.radians(LONLAT[1])))**2 + (mY - LONLAT[1])**2))
...
axes[0].plot(narT.index, narT.values, '-', color='darkorange', lw=2.0, drawstyle='steps-post', label='NAR (Swan) inflow T (BC, raw 21.6)')
...
fig.suptitle(f'{SITE} ({LONLAT[0]}, {LONLAT[1]}) - model vs SMCWS field: surface & bottom T, S, density\n'
             f'sim start 22-Feb -> last field cast {last:%d-%b} 1992 (1992 rev; RAW inflow BC); T panel shows NAR inflow T', fontweight='bold')
