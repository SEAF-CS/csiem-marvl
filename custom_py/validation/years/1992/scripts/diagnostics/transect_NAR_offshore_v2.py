...
Snapshot at the 25-Mar TransectOA time. -> years/1991/outputs/diagnostics/.
"""
...
OUT = r'Q:/SEAF-CS/V1.8/MARVL/csiem-marvl/custom_py/validation/years/1992/outputs/diagnostics/transect_NAR_offshore_v2.png'
SURVEY_LOG = r'Q:/SEAF-CS/V1.8/DATA/csiem-data/data-lake/DEP/SMCWS/1992/survey_log_1992.csv'
PROFILE_BASE = r'Q:/SEAF-CS/V1.8/DATA/csiem-data/data-lake/DEP/SMCWS/1992'
MODEL_TIME = pd.Timestamp('1992-03-25 12:00')   # March survey snapshot
MODEL_ONLY = True                                # 1992 mock-up: 1991-specific field loader skipped
SLEV = np.arange(24, 36.6, 0.5); CMAP = plt.cm.RdYlBu_r   # 1992 autumn: saltier estuary range
...
ds=xr.open_dataset(NC); fv=ds.tfv   # RHOW not needed here (only SAL used); avoid full-field RHOW OOM
...
for d,lo,la in dens:
    ...
    try:
        p=fv.get_profile((clo,cla),variables=['SAL','TEMP'],time=md)
...
casts18=[]
for row in ([] if MODEL_ONLY else csv.DictReader(open(SURVEY_LOG))):
...
fig.suptitle(f'NAR->offshore along-thalweg salinity: MODEL vs FIELD (1992 rev RAW-BC)  |  {md:%d-%b %H:%M}',fontweight='bold')
