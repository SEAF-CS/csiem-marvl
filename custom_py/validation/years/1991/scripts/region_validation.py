"""1991 BASELINE vs REVISED model — obs-vs-model salt-bias comparison (the Aug-1991 survey).
Component [1] region_validation, for year 1991.  Same casts, two paired models:
  - baseline : output_archive/1.7.0/1991_aug/csiem_B010_19910720_19910831.nc
  - rev      : output_archive/1.7.0/1991_aug_rev/csiem_B010_19910720_19910831_rev_repaired.nc
               (graded-FRESH IC 35.4->34.0 + Mandurah PHE + poly-6 OBC corr + SDOOL re-enabled
                + O2Me-AHD bathy overrides: Success channel deepened + Minstrel sill)
Writes region_validation_1991_{base,rev}.csv + _revcompare.{csv,png} to this year's outputs/.
...
OUT = os.path.abspath(os.path.join(HERE, '..', 'outputs'))   # validation/years/1991/outputs
os.makedirs(OUT, exist_ok=True)
WIN = ('1991-07-20', '1991-08-31')
REV_NC = r'S:/Matt_Working/csiem/output_archive/1.7.0/1991_aug_rev/csiem_B010_19910720_19910831_rev_repaired.nc'
REGIONS = ['OA', 'CS', 'N', 'NW', 'W', 'SW', 'S']

print('=== BASELINE 1991 extraction ===')
df_base = core.run(WIN)
df_base.to_csv(os.path.join(OUT, 'region_validation_1991_base.csv'), index=False)

print('\n=== REV 1991 extraction ===')
core.FV_RUNS[0] = ('1991-07-20', '1991-08-31', REV_NC)   # repoint the 1991 entry at the rev NC
core._FV_CACHE.clear()
df_rev = core.run(WIN)
df_rev.to_csv(os.path.join(OUT, 'region_validation_1991_rev.csv'), index=False)
...
print('\n=== HEADLINE: 1991 surface salinity bias (model - obs) ===')
...
fig.suptitle(f'1991 Aug — obs-vs-model salt/temp bias: BASELINE vs REVISED (fresh IC + Mandurah PHE + poly-6 OBC + SDOOL + bathy override)\n'
             f'overall surface-S bias  {ob:+.3f} → {orv:+.3f} psu   (n≈{nb}; + = model too salty)',
             fontsize=12.5, fontweight='bold')
fig.tight_layout(rect=[0, 0, 1, 0.95])
p = os.path.join(OUT, 'region_validation_1991_revcompare.png')
fig.savefig(p, dpi=150); plt.close(fig)
pd.DataFrame(recs).to_csv(os.path.join(OUT, 'region_validation_1991_revcompare.csv'), index=False)
print('\nwrote', p)
