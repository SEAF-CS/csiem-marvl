"""CS55 surface & bottom time series: model T, S, rho over the sim vs SMCWS field casts (dots).
From sim start (20-Jul) to the last CS55 field measurement (23-Aug 1991).
...
OUT = r'Q:/SEAF-CS/V1.8/MARVL/csiem-marvl/custom_py/validation/years/1991/outputs/TimeSeries/cs55_timeseries_TSrho_1991_rev.png'
import os as _os; _os.makedirs(_os.path.dirname(OUT), exist_ok=True)
CS55 = (115.7140, -32.1876)
SIM0 = pd.Timestamp('1991-07-20')

# ---- model: full profile time series at CS55, compute surf/bot per timestep ----
...
Z = -np.asarray(p['Z']).reshape(len(mt), -1)      # depth positive down
...
for i in range(len(mt)):
    ...
    msS[i], msT[i] = s[su].mean(), t[su].mean()
    mbS[i], mbT[i] = s[bo].mean(), t[bo].mean()
msR = eos80_potential_density(msS, msT) - 1000.0
mbR = eos80_potential_density(mbS, mbT) - 1000.0

# ---- field: CS55 casts in the sim window ----
inv = pd.read_csv(os.path.join(DATA, 'region_profiles_inventory.csv'))
cs = inv[(inv.station == 'CS55') & (inv.year == 1991)].copy()
...
for _, r in cs.iterrows():
    ...
    fsR.append(eos80_potential_density(o['surfS'], o['surfT']) - 1000.0)
    fbR.append(eos80_potential_density(o['botS'], o['botT']) - 1000.0)
fdt = pd.to_datetime(fdt)
last = fdt.max(); print(f'CS55 field casts in sim window: {len(fdt)}  ({fdt.min():%d-%b} .. {last:%d-%b %H:%M})')

# ---- forcing: river inflow (NAR x1.5 + CAN x2.0, as applied) + CS wind ----
def load_flow(tag, scale):
    ...
    f['t'] = pd.to_datetime(f['Date'], format='%d/%m/%Y'); return f.set_index('t')['Flow'] * scale
nar = load_flow('NAR', 1.5); can = load_flow('CAN', 2.0)
flow = (nar + can); flow = flow[(flow.index >= SIM0 - pd.Timedelta('1d')) & (flow.index <= last + pd.Timedelta('2d'))]
wind = pd.read_csv(r'Q:/SEAF-CS/V1.8/MARVL/csiem-marvl/custom_py/mesh_refinement/common/data/wind_cs_1991.csv')
wind['t'] = pd.to_datetime(wind['time']); wind = wind.set_index('t')

# ---- plot: 4 rows (T, S, rho, forcing), surf/bot model lines + field dots ----
SURF, BOT = '#1f77b4', '#d62728'
rows = [('Temperature (°C)', msT, mbT, fsT, fbT),
        ('Salinity (psu)',        msS, mbS, fsS, fbS),
...
# ---- west-most IC value (ocean model-limit end of the graded IC) as a left-edge reference ----
west_ic = [19.4, 36.10, float(eos80_potential_density(36.10, 19.4) - 1000.0)]   # [T, S, rho]
for ax, wv in zip(axes[:3], west_ic):
    ax.plot(SIM0, wv, marker='*', ms=15, color='m', mec='k', mew=0.6, zorder=7, clip_on=False)
    ax.annotate(f'IC W-limit {wv:.2f}', (SIM0, wv), xytext=(-8, 0), textcoords='offset points',
                fontsize=7.5, va='center', ha='right', color='m', fontweight='bold')

# ---- panel 4: river inflow (primary y) + CS wind speed (secondary y) ----
axf = axes[3]; FC = '#1f5c8a'; WC = '#2ca02c'
axf.fill_between(flow.index, flow.values, step='mid', color=FC, alpha=0.30, zorder=1)
axf.plot(flow.index, flow.values, drawstyle='steps-mid', color=FC, lw=1.4, label='river inflow (NAR×1.5 + CAN×2.0)')
axf.set_ylabel('River inflow (m$^3$ s$^{-1}$)', color=FC); axf.tick_params(axis='y', labelcolor=FC)
axf.set_ylim(0, None); axf.grid(alpha=0.3)
axw = axf.twinx()
axw.plot(wind.index, wind['speed'], color=WC, lw=0.8, alpha=0.85, label='CS wind speed')
axw.set_ylabel('Wind speed (m s$^{-1}$)', color=WC); axw.tick_params(axis='y', labelcolor=WC); axw.set_ylim(0, None)
h1, l1 = axf.get_legend_handles_labels(); h2, l2 = axw.get_legend_handles_labels()
axf.legend(h1 + h2, l1 + l2, fontsize=8, loc='upper left')
...
fig.suptitle('CS55 (115.714, -32.188) - model vs SMCWS field: surface & bottom T, S, density\n'
             'sim start 20-Jul -> last field cast 23-Aug 1991 (1991 rev)', fontweight='bold')
